        ...
        for ext in [".jpg", ".jpeg", ".png", ".tif", ".tiff", ".bmp", ".webp"]:
            image_files.extend(
                [
                    ...
                    if f.lower().endswith(ext) and os.path.isfile(os.path.join(subfolder_path, f))
                ]
            )
            ...
            try:
                with Image.open(original_path) as img:
                    ...
                    if img is None or img.size == (0, 0):
                        logger.warning(f"Skipping '{original_filename}' due to invalid size after trim.")
                    ...
                    if needs_resize:
                        ...
                        img = img.resize((new_width, new_height), get_resampling_filter())
                    ...
                    if os.path.abspath(target_path) != os.path.abspath(original_path):
                        if try_remove(original_path):
                            ...
                            logger.warning(f"Failed to remove original file: {original_path}")

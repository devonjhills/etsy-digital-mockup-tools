...
def create_video_mockup(
    ...
    # Create a second video writer for the videos folder
    video_writer_etsy = cv2.VideoWriter(
        videos_output_path,
        fourcc,
        fps,
        target_size
    ...
    if not video_writer.isOpened() or not video_writer_etsy.isOpened():
        logger.error(f"Failed to open video writer for {output_path} or {videos_output_path}")
        return False
    ...
    try:
        for i in range(len(cv_images)):
            ...
            for _ in range(display_frames):
                ...
                video_writer_etsy.write(current_img)
            ...
            for j in range(transition_frames):
                ...
                video_writer_etsy.write(blended)
        
        video_writer.release()
        video_writer_etsy.release()
        
        success = os.path.exists(output_path) and os.path.getsize(output_path) > 0
        success_etsy = os.path.exists(videos_output_path) and os.path.getsize(videos_output_path) > 0
        
        if success and success_etsy:
            logger.info(f"Video created successfully: {output_path}")
            logger.info(f"Video also saved for Etsy: {videos_output_path}")
            return True
        elif success:
            logger.info(f"Video created successfully: {output_path}")
            logger.warning(f"Failed to create video for Etsy: {videos_output_path}")
            return True
        else:

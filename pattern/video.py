...
def create_seamless_zoom_video(input_folder: str, seamless_image_path: Optional[str]) -> Optional[str]:
    """
    ...
    if not seamless_image_path or not os.path.exists(seamless_image_path):
        logger.warning(f"Seamless pattern image '{seamless_image_path or 'None'}' not found. Skipping zoom video.")
        return None
    ...
    try:
        ...
        # Create a copy in the mocks folder
        try:
            import shutil
            mocks_video_path = os.path.join(mocks_folder, "video_seamless.mp4")
            shutil.copy2(video_path, mocks_video_path)
            logger.info(f"Seamless zoom video saved to videos folder: {video_path}")
            logger.info(f"Seamless zoom video copied to mocks folder: {mocks_video_path}")
        except Exception as e:
            logger.error(f"Error copying video to mocks folder: {e}")

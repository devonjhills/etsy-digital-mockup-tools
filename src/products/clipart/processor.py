...
class ClipartProcessor(BaseProcessor):
    ...
    def _create_square_mockup(self) -> Dict[str, Any]:
        ...
        try:
            ...
            all_files = find_files_by_extension(
                self.config.input_dir, [".png", ".jpg", ".jpeg"]
            )
            image_files = [
                f for f in all_files if "/mocks/" not in f and "\\mocks\\" not in f
            ]
            ...
            subtitle_bottom = '300 DPI • 5" • Transparent PNG'
    ...
    def _create_grid_mockup(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if len(image_files) < 4:
                return {
                    "success": False,
                    "error": f"Need at least 4 images for grid, found {len(image_files)}",
                }
            ...
            self.logger.info(
                f"Total images: {len(image_files)}, will create {num_grids} grids"
            )
            ...
            for grid_num in range(num_grids):
                ...
                if result_path:
                    ...
                    if watermarked_path:
                        ...
                        self.logger.info(
                            f"Created and watermarked grid {grid_num + 1}/{num_grids}: {watermarked_path}"
                        )
                    else:
                        created_files.append(result_path)
                        self.logger.warning(
                            f"Grid created but watermarking failed for grid {grid_num + 1}"
                        )
                else:
    ...
    def _create_transparency_demo(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if not image_files:
                return {
                    "success": False,
                    "error": "No images found for transparency demo",
                }
            ...
            demo_image = create_transparency_demo(
                image_path=demo_image_path, scale=0.7, checkerboard_size=30
            )
    ...
    def _create_pinterest_mockup(self) -> Dict[str, Any]:
        ...
        try:
            ...
            result_file = mockup_processor.create_pinterest_mockup(
                self.config.input_dir, title, product_data
            )
    ...
    def extract_from_sheets(self) -> Dict[str, Any]:
        ...
        try:
            self.logger.info(
                f"🔍 Starting clipart extraction from: {self.config.input_dir}"
            )
            ...
            result = extract_clipart_from_sheets(
                input_folder=self.config.input_dir, output_folder=extracted_dir
            )

            if result.get("success"):
                count = result.get("extracted_count", 0)
                self.logger.info(
                    f"✅ Extraction completed: {count} clipart images extracted"
                )
